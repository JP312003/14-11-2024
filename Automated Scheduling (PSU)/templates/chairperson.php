...
$sql_user = "SELECT name, role FROM users WHERE user_id = ?";
...
// SQL query to fetch instructor ID and name for the chairperson's department
$sql_instructors = "SELECT i.instructor_id, i.instructor_name
                    FROM instructors i
...
$stmt_instructors->bind_param("i", $department_id);
...
<!-- -------------------------------------------------------------------------------------------------------------------- -->
    ...
    <script src="../statics/js/chairperson.js"></script>
    <?php
    // Close connection
    $conn->close();
    ?>
